...
- Fixed PR #88 for Argon2 0.6: migrated password hashing to the new random-salt API, removed the obsolete direct `rand_core` dependency, and added new-hash and existing-PHC compatibility tests with explicit CodeQL test-fixture suppressions. Verified formatting, 149 tests, Clippy with warnings denied, the locked dependency audit, diff checks, and a clean release container build.
